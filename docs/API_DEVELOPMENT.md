...
这些调度入口目前仍在服务器控制线程的任务阶段执行，提供目标绑定、有效性检查和取消语义。
原生方块实体阶段已另行接入区域工作线程；不能凭调度入口或线程名字推断当前拥有某个区域。
`runMain*` 保持现有行为；旧 `region()` 仍是后台任务分道，不拥有世界数据。
...
`regionSimulation.workers` 控制已接入原生区域阶段的并行度：默认 `0` 自动选择 2–8 个工作线程，
`1` 保留串行路径。修改后需要重启；配置重载会报告该字段需要重启。
当前接入熔炉、高炉、烟熏炉、酿造台、营火及已完成战利品初始化的漏斗路径。同一批内只有一个可执行区域时直接串行推进；
含尚未接入的原生 ticker 的区域在控制线程处理，其他独立区域可以并行。

相关烧炼、酿造及燃料、漏斗搬运/拾取、方块状态及物理事件可能在 `Fand Region #...` 上调用。
事件接口和取消机制保持不变；不同区域的监听器可能同时执行，插件共享数据需要自行协调。
事件对应位置的 `Block` 查询、修改和组件读写可同步使用，但整个同步调用链都必须保持在当前区域的已加载范围内。
区域许可绑定本次调用及世界实例，不能缓存许可，也不能把获得的可变库存或实体随意交给其他区域使用。

全局查询或跨区域操作使用 `scheduler.global()`、`scheduler.at(...)`、`scheduler.forEntity(...)` 提交，
通过 Future 的后续操作处理结果。区域回调中不要 `join()` 或 `get()` 等待这些任务：控制线程正在等待本批区域完成。
同步的全局线程桥和跨区域方块访问会明确失败，不会偷偷加载区块或等待主线程。
世界时间、天气等仍由控制阶段推进；原生计划刻仍使用世界时间，区域方块实体子阶段不推进完整区域时钟。

区域内经原生容器掉落入口产生物品时，源物品栈立即扣除，独立快照在本批区域全部完成后创建并注册物品实体。
因此本批后续漏斗不会提前拾取尚未发布的新掉落物；下一次模拟即可访问。
酿造音效、营火游戏事件也在阶段结束后按区域内部调用顺序分发，避免工作线程修改玩家广播或振动监听器索引。
事件接口、取消及修改酿造结果的同步语义保持不变。

方块计划刻也已接入同一派发器，目前允许红石灯的延迟熄灭回调在独立区域并行执行。
含未迁移方块计划刻的区域批次仍在控制线程执行；中继器、比较器、侦测器和流体回调暂未并行。
本轮任务先按原有优先级及世界级上限选定，同一区域内保持选定顺序；回调中新登记的到期任务留到下一轮。
不足两个可并行区域时，串行回退保留本轮原始选定顺序，不因区域分组重新排序。
区域内的计划刻查询、清除和复制必须全部位于当前区域；复制同时检查源与目标范围，跨世界及跨区域操作改用所有者调度。
同一世界的计划刻阶段与方块实体阶段依次执行，不会让两者同时修改同一份区块数据。

区域回调中的原生方块事件登记及范围清除也检查世界和所有权；清除前检查整个范围，越界不会部分删除。
事件执行、区块卸载和世界级队列诊断仍属于控制阶段，回调新登记的方块事件仍可在同一趟事件执行中被处理。

原生 TNT 点燃入口在区域内同步判断游戏规则并返回点燃结果，调用方仍立即移除本区域的 TNT 方块。
`PrimedTnt` 构造、实体注册、点燃声音及游戏事件在所有区域结束后按本区域顺序发布，
因此实体可见性与 `EntitySpawnEvent` 相对区域回调会延后到阶段结束，生成事件本身仍在控制线程执行。
已接受的点燃不会因发布前另一个回调修改游戏规则而丢失；生成事件取消注册时，仍沿用原有点燃返回值与方块消耗语义。
这一入口的改造不代表爆炸链、所有红石计划刻或通用实体注册已经并行。
