# 动态分区实施记录：P0/P1

更新日期：2026-09-24

对应方案：[动态分区与多线程化企划案](REGIONIZED_MULTITHREADING_PROPOSAL.md)

## 当前完成范围

已实现**现有串行执行模型上的目标调度入口**，以及**可独立执行的分区拓扑、holder 生命周期适配、区域执行器与局部 tick 队列**。
第四批将 holder 生命周期和区块异步安装身份校验接入真实 NMS 路径；第五批将实体与方块实体 tick 成员按 cell 存放，并接入移动及卸载生命周期。
第六批继续接入方块/流体计划刻和 block events，并处理执行批次、保存预留及同步邻居更新链。
第七批将区域执行器接入真实 NMS 方块实体阶段：独立区域中的熔炉、高炉、烟熏炉与漏斗可在不同工作线程上执行原生回调。
第八批扩展到酿造台与营火，并将容器掉落实体注册、音效和游戏事件分发接入阶段结束后的发布队列。
第九批提取通用原生区域派发器，将方块计划刻的选定与执行分开，接入红石灯计划刻并行及区域范围访问检查。
`MinecraftServer` 的世界控制流程、实体、随机刻及计划刻执行阶段仍未全部区域化；当前是带阶段屏障的实验并行，尚非各区域独立 TPS 的完整实现。
计划刻沿用世界游戏时间，区域方块实体子阶段不推进完整区域时钟。P0/P1 整体尚未完成。

新增公开入口：

```java
scheduler.at(location).run(action);
scheduler.forEntity(entity).call(query);
scheduler.global().run(action);
```

返回类型与使用规则见 [API 开发文档](API_DEVELOPMENT.md#所有者调度)。新增方法使用 `default`，不要求旧 `Scheduler` 实现立即补充抽象方法；
旧实现未提供这些能力时明确报告不支持，不会偷偷改用后台线程。
本批只增加单次调用，延迟、重复调度和区域所有权查询还没有作为公共 API 发布。

## 实际 tick 调用链

以下顺序来自当前 26.2 源码，不是未来区域架构的理想调用图：

```text
MinecraftServer.processPacketsAndTick()
  packetProcessor.processQueuedPackets()
  FandServer.tickScheduler()
    TaskScheduler.tick()
    recipes.tick(), maps.tick(), modProtocols.tick()
    commandTrees.tick() -> 给玩家发送更新后的指令树
    遍历世界 -> customBlocks.tick(world)
  MinecraftServer.tickServer()
    处理空服暂停、服务器 tick 计数和 tickRateManager
    MinecraftServer.tickChildren()
      commandFunctions、clockManager、时间同步
      遍历世界 -> ServerLevel.tick()
        世界时间、天气/睡眠
        方块计划刻 -> 世界预算选定 -> 控制组执行 -> 已接入区域并行 -> 屏障与发布
        流体计划刻（控制线程）
        袭击、区块模拟、block events
        龙战、实体
        方块实体 -> 控制组执行 -> 独立区域工作线程并行 -> 全部结束后发布管理通知
        实体管理
      连接、玩家列表、调试与其他 tickable
      区块发送与网络 flush
    状态发布、自动保存与统计
```

源码：[MinecraftServer.java](../fand-server/src/minecraft/java/net/minecraft/server/MinecraftServer.java)、
[FandServer.java](../fand-server/src/main/java/io/fand/server/FandServer.java)、
[ServerLevel.java](../fand-server/src/minecraft/java/net/minecraft/server/level/ServerLevel.java)。

发现一个不能忽略的时钟边界：Fand 任务先于 `tickServer()` 的空服暂停判断运行。
因此，当前调度器 tick 不等于一次已经完成的世界模拟 tick。未来添加区域延迟时必须明确区分二者，不能直接复用同一计数器。
本批的新单次任务复用现有调度顺序，不改变这个已有行为。

## 首批共享状态清单

| 位置与状态 | 当前归属 | 区域化前必须解决的事项 |
| --- | --- | --- |
| `ServerLevel.entityTickList`、玩家列表 | 世界线程；tick 成员已分 cell | 玩家列表、乘客共同执行归属、并行移交和去重 |
| `ServerLevel.blockTicks` / `fluidTicks` | 世界线程；区块容器与截止时间索引已分 cell | 拆分/合并时转换局部截止时间，领取批次绑定区域许可 |
| `ServerLevel.blockEvents` | 世界线程；事件已分 cell，延后列表属于当前执行批次 | 保持同 tick 副作用，关联跨 cell 同步更新范围 |
| `Level.blockEntityTickers` 内部 active / pending | 成员交接使用短锁；已接入的原生回调归区域所有者 | 扩展其余 ticker，保留同区顺序和阶段边界 |
| `Level.neighborUpdater` | 控制阶段一个实例，区域阶段每个所有者一个实例 | 扩大已验证的同步更新范围，补其余红石共享状态审计 |
| `Level.random` 使用 `ThreadUnsafeRandom` | 控制阶段私有；区域阶段 `getRandom()` 使用作用域随机源 | 后续随机刻/刷怪接入时迁移仍直接读取字段的路径 |
| `Level.thread` | 控制线程身份；区域方块实体访问另检查原生作用域 | 扩展实体和其他 API 的执行所有权检查 |
| `ServerChunkCache` 距离管理、票据、`spawningChunks`、刷怪统计 | 世界线程 | 加载状态发布、临时列表分区和世界总额协调 |
| `ServerLevel` 爆炸缓存、每 tick TNT 额度 | 世界线程 | 缓存覆盖范围/生命周期与全世界额度含义 |
| 世界时间、`clockManager`、袭击、龙战、睡眠 | 世界/服务器控制流程 | 每项确定唯一决策者，避免每区重复更新 |
| Fand 自定义方块和组件 | 全局任务中逐世界运行 | 按方块位置迁移 tick 与组件访问 |
| 配方、地图、协议、指令树更新 | Fand 全局任务 | 注册表版本发布与按玩家所有者发送分开 |

补充源码：[Level.java](../fand-server/src/minecraft/java/net/minecraft/world/level/Level.java)、
[ServerChunkCache.java](../fand-server/src/minecraft/java/net/minecraft/server/level/ServerChunkCache.java)。
这是入口级审计，不是整个 NMS 的共享状态审计已经完成。第二批继续检查的发布入口如下：

| 系统 | 确认的共享状态/完成入口 | 接入要求 |
| --- | --- | --- |
| `ChunkMap` | `updatingChunkMap`、`visibleChunkMap`、`pendingUnloads`、实体跟踪和玩家预加载引用；`scheduleChunkLoad()` 在后台解析后回 `mainThreadExecutor` 安装区块 | 区分后台计算和实时对象发布；区块及 POI 安装必须在合法所有者中执行 |
| `ChunkMap.scheduleUnload()` | 等待保存同步 Future，再经 `unloadQueue` 移除 holder、保存、卸载世界区块并更新光照 | 保留 holder 身份检查，协调 cell 最后一个活动区块的撤销和在途保存 |
| `DistanceManager` | `playersPerChunk`、票据集合、距离传播与 `ChunkHolder.updateFutures()`；部分异步完成后再次投递主线程 | 分离世界级统计和按位置状态；不能只换成并发 Map 后让多个区域共同更新 |
| `PoiManager` | `loadedChunks`、村庄距离追踪、`take()` / `release()` 与脏状态发布 | POI 占用和修改随空间所有者推进；存储层加载完成不等于可跨线程直接调用 |
| `PersistentEntitySectionManager` | UUID 集合、区块可见性、加载状态和 `loadingInbox`；`processPendingLoads()` 安装实体并更新状态 | 并发队列只保护交接；消费时的实体安装、去重和区块状态必须一起归属到正确执行域 |

源码：[ChunkMap.java](../fand-server/src/minecraft/java/net/minecraft/server/level/ChunkMap.java)、
[DistanceManager.java](../fand-server/src/minecraft/java/net/minecraft/server/level/DistanceManager.java)、
[PoiManager.java](../fand-server/src/minecraft/java/net/minecraft/world/entity/ai/village/poi/PoiManager.java)、
[PersistentEntitySectionManager.java](../fand-server/src/minecraft/java/net/minecraft/world/level/entity/PersistentEntitySectionManager.java)。

另发现 `ChunkMap.handleChunkLoadFailure()` 对部分失败会继续创建空区块。它不能直接作为区域所有权失败的兜底策略；
解析失败、已有存档损坏和过期所有者结果应分别处理。本批记录该路径，没有改动既有区块恢复行为。

## API 调用分类与迁移顺序

| 类别 | 当前实例 | 后续处理 |
| --- | --- | --- |
| 所有者内可同步完成 | 方块读写、当前实体状态与库存操作 | 保持主体签名，把线程许可改为目标所有权 |
| 已有 Future 的操作 | 玩家传送、世界时间修改、部分实体动作 | 保留签名，逐项接入目标所有者和生命周期协议 |
| 阻塞投递 | `ServerThreading.callBlocking()`、`FandBlock.callOnServerThread()`、部分玩家方法的 `join()` | 明确同步调用范围，跨所有者提供异步替代 |
| 自动投递的 `void` 操作 | `FandEntity.runOnServerThread()` 和玩家/方块包装器 | 明确调用是立即完成还是只接受请求，避免悄悄改变行为 |
| 只读快照 | 玩家/世界列表和区块跟踪数据 | 区分不可变集合与其中可变对象；不自动授予所有权 |
| 必须单独设计 | 跨区库存、命令链、邻居更新、传送与存档 | 先定义原子边界，再接入区域调度 |

源码：[ServerThreading.java](../fand-server/src/main/java/io/fand/server/util/ServerThreading.java)、
[FandBlock.java](../fand-server/src/main/java/io/fand/server/block/FandBlock.java)、
[FandEntity.java](../fand-server/src/main/java/io/fand/server/entity/FandEntity.java)、
[FandPlayer.java](../fand-server/src/main/java/io/fand/server/entity/FandPlayer.java)。

`FandBlock` 还存在“server 为 null 就直接在调用线程执行”的分支；它不能被区域实现继承为缺失所有者时的兜底。
本批没有大面积改写这些原有 API，以免把尚未落实的区域语义提前带入现有玩法。

## 第一批：目标调度的约束

- 所有者任务与现有主线程任务共用 tick 队列，保持提交顺序；回调中提交的任务留到下次 tick。
- `ServerTickAccess` 在服务端执行线程上检查目标；任务提交线程不读取实时实体位置或世界注册表。
- 位置任务绑定世界实例。卸载后同 key 的新世界不能继承旧任务。
- 实体任务检查当前句柄、世界实例、移除状态与实体索引中的身份，不依赖 UUID 相等来恢复旧对象。
- 玩家包装器重新绑定时读取当前句柄；普通实体若被原版替换为新实例，旧包装器退休。跨实例身份移交仍属于后续 NMS 设计。
- Future 失败和取消对调用者可见。调度器关闭会结束待处理 Future，不能遗留永远不完成的查询。
- 通过插件上下文提交的查询进入资源追踪，结束后释放；停用后拒绝新提交，并在执行入口阻止尚未进入的动作。
- 取消回调在队列锁/资源锁外调用，避免回调再次调度时产生锁等待循环。
- 本批的停用语义仍是尽力取消；已经进入的插件回调不保证被排空。完整的在途计数、生命周期代际与类加载器静止协议尚未实施。

## 第二批：拓扑、执行上下文与 cell 任务队列

实现位于 [tick 包](../fand-server/src/main/java/io/fand/server/tick/TickRegionTopology.java)，未增加公共 `fand-api` 接口。

### 网格与布局

- `OwnershipCell` 使用固定网格坐标，区块坐标转换采用向下取整，覆盖负坐标。
- 每个已加载世界应拥有独立的 `TickRegionTopology` 实例。区域身份不跨该实例复用，同一个数值 ID 不代表同一个区域。
- 每个活动 cell 周围有配置好的缓冲范围；缓冲重叠的部分归入同一区域。缓冲 cell 本身不会因此变成活动 cell。
- `tryActivate()` 合并受影响的空闲区域；`tryDeactivate()` 只对受影响区域重新计算连通分量并拆分。其他区域的身份保持。
- 区域正在执行时，相关变更返回 `BUSY`，整个布局保持原样；holder 适配器会保留变更，后续维护 pass 按预算重试。当前没有动态负载策略。
- 重复激活或撤销非活动 cell 返回 `UNCHANGED`。缓冲坐标溢出在发布之前失败，不会绕回负坐标并修改其他区域。

模型的激活单位是 **cell**。未来区块适配器必须根据同一 cell 中的区块 holder 生命周期聚合首个加入与最后一个退出，
不能每卸载一个区块就撤销整个 cell。第三批已实现该适配器，第四批已挂接真实 NMS 生命周期。
缓冲半径是模型参数，测试中的数值不是对 NMS 所有访问范围的安全证明，也尚未加入服务器配置。

### 执行上下文

`RegionContext` 是绑定当前线程的一次执行许可。进入时区域从 `READY` 变为 `RUNNING`，释放后才能进行布局调整。
不同区域可以由不同线程同时执行；同一区域不能同时进入，一个线程也不能同时持有两个区域。
上下文退出后不能继续检查/使用其所有权，旧上下文重复关闭不会释放后续执行获得的新许可。

拓扑替换创建新区域身份，旧身份变为 `RETIRED`，不能再次进入。关闭整个拓扑要求先释放所有执行许可，随后退役全部区域。
这个上下文目前只保护模型内的归属与任务执行；NMS 仍使用原来的线程模型，不得凭模型许可绕过现有线程限制。

### 任务路由与生命周期

任务存放在活动 cell 的队列中，执行区域在领取任务时确定。拆分或合并只更新所有权布局，保留仍活动 cell 的队列，
不搬运 Future、不保存旧区域转发链。跨 cell 合并后的任务按原提交序号执行。

撤销 cell 时，旧活动实例的待执行任务取消。重新激活同一坐标建立新队列，即使取消回调中立即重新激活，也不会复活旧任务或误取消新任务。
一次执行上下文只领取一批任务；执行期间的新提交留给下一次执行。取消会从待执行队列释放对应任务。

普通任务异常进入其 Future；严重 `Error` 会结束已经领取的剩余结果并向执行者抛出。
释放失败上下文时，区域转为 `FAILED`，未领取的任务也失败，后续执行和拓扑修改被阻止。
第三批的 `runTick()` 将此边界扩展到整个模拟回调；模拟回调中的普通异常也会使区域失败。
模型不尝试回滚回调已经产生的副作用，真实 NMS tick 仍需通过该执行边界接入。

布局发布和队列交接使用同一把短期元数据锁；任务本体和完成/取消回调在锁外执行。
当前连通分量重算和缓冲集合构建仍在该锁内，虽然仅涉及受影响区域，但成本会随大区域增大。
大规模 cell 的开销、调度公平性、背压与负载自适应需要后续基准验证，本批不宣称已解决它们。

## 第三批：holder 生命周期、执行派发与局部时钟

### holder 生命周期与异步结果

[ChunkOwnership](../fand-server/src/main/java/io/fand/server/tick/ChunkOwnership.java) 聚合同一 cell 的多个区块 holder，
维护线程固定为创建它的控制线程。其他线程只能提交注册、退休请求，或通过已激活的 handle 提交动作。

- `register(chunkX, chunkZ, holder)` 创建一次独立生命周期；同一坐标甚至同一 holder 对象再次注册，仍生成新身份。
- `ready()` 完成后才允许通过该 handle 投递。等待期间不得提前把新 holder 发布为实时可访问的区块。
- 第一个 holder 激活 cell，最后一个 holder 完成退休才撤销 cell。替换 holder 不需要重建仍活动的 cell 队列。
- `retire()` 立即阻止该身份接受新动作，实际撤销等所有者空闲。已经进入的动作允许结束，不执行线程中断。
- 同一坐标的新注册替换旧身份；旧身份迟到的退休请求不能卸载新 holder。
- `handle.submit()` / `submitAfter()` 的动作在执行时再次校验身份。退休也会取消尚未执行的该 holder 任务，即使 cell 仍有其他区块。
- 注册/退休返回的是观察 Future 的副本，取消副本不会偷偷取消区块生命周期操作。

`drain(maxRequests, maxChanges)` 分别限制读取请求数和尝试变更数。忙坐标轮转到末尾，其他独立区域仍可处理；
每个 pass 不阻塞等待区域，也不在循环内反复重试同一忙坐标。这限制的是工作项数量，单次大型连通分量重算仍可能较贵。

维护期间短暂停止新执行许可的领取，已有区域继续执行；涉及运行中区域的成员变化一律延期，包括同一 cell 内的 holder 增减。
此约束使 holder 身份切换与拓扑更新处于同一安全点。完成和取消通知在维护门禁与元数据锁之外运行，允许继续提交请求。

`shutdown()` 立即停止新执行许可和任务接收。控制线程继续 `drain()`，直到已接受请求收尾且所有执行许可释放，
再退休全部 holder、取消未执行动作并结束关闭 Future。失败区域也允许通过世界关闭统一回收。

### 区域执行与失败边界

[RegionTickExecutor](../fand-server/src/main/java/io/fand/server/tick/RegionTickExecutor.java) 使用调用方提供的工作线程池，
对每个区域身份最多保留一个待执行或执行中的 tick。不同区域之间没有全部完成后才能继续的屏障。

领取时重新检查区域身份：拓扑已经替换、区域忙、维护门禁关闭或世界正在停止时返回 `UNAVAILABLE`，交给控制流程重试。
它不依赖数值 region ID 区分世界；不同世界相同 ID 不会共用任务。
执行器关闭取消尚未开始的 tick，并等待已开始 tick 释放许可，不负责关闭调用方的共享线程池。

`RegionContext.runTick()` 先消费到期队列，再运行一次区域模拟。成功释放后才推进局部计数；
模拟失败会使整个区域进入 `FAILED`，未领取任务失败，时钟不前进。执行结果通知发生在释放区域许可之后。
单独调用 `runTasks()` 只执行队列，不代表完成一次世界模拟，因此不会推进时钟。

### 局部 tick 延迟

`submitAfter(cell, delayTicks, action)` 使用正整数局部 tick 延迟。运行中的 tick 内提交延迟，从该 tick 结束后开始计数。
合并选择来源区域中最大的局部计数，并按 `newDue = oldDue + targetNow - sourceNow` 平移每个来源的截止 tick；
拆分后的子区域继承父区域时钟，随后各自推进。截止时间溢出会在布局发布前失败，原所有权与队列保持完整。

cell 的队列按截止 tick 和提交序号排序，领取时只读取到期部分；不会每 tick 扫描所有远期任务。
取消从队列移除任务；合并时在重新索引之前完成全部截止时间验证。
这套时钟尚不包含墙钟延迟、世界游戏时间、冻结/变速与实体移交，也没有发布为新的公共 API。

### 接入边界

目前可以组合 holder 适配器、区域执行器与模型回调验证并行、合并/拆分和关闭。但**不能将回调直接替换成 `ServerLevel.tick()`**：
原版实体、计划刻、POI、距离管理等状态仍归世界线程。现有 `Scheduler.at()` / `forEntity()` 继续使用原串行实现。
执行器也尚未提供自动 20 TPS 定时、公平队列、队列总量背压或按负载自适应的区域调整。

## 第四批：接入真实 NMS holder 与异步加载

新增内部 [SerialChunkOwnership](../fand-server/src/main/java/io/fand/server/tick/SerialChunkOwnership.java)，
将模型生命周期挂接到 `ChunkMap`，使用现有世界控制线程完成注册、撤销和数据安装。
拓扑保持私有，不向区域工作线程开放执行许可。当前 cell 为 4 × 4 区块、缓冲半径为 1 个 cell，
仅用于记录真实 holder 对应的连通布局，不能据此认定原版邻域访问已经安全分区。

- 新建 `ChunkHolder` 时，先完成身份绑定，再放入 `updatingChunkMap`。从 `pendingUnloads` 取回旧 holder 时保留原身份。
- `scheduleUnload()` 的旧回调只有成功移除对应 holder 才能撤销身份；等待保存期间仍保持所有权。
  最终卸载即使没有生成出 chunk，也会撤销身份；有 chunk 时先完成保存提交和原版卸载操作。
- `scheduleChunkLoad()` 捕获本次 holder 的身份，后台继续读取和解析数据、预取 POI；回到原世界事件循环后，
  只有该身份仍然有效才执行安装或原有读取失败恢复。不会让旧结果写入同坐标的新 holder。
- 世界关闭先停止接收安装，再退休 holder。尚未完成 I/O 或已经排队的安装 Future 均会结束；
  未完成的来源 Future 只保留可解除的回调，退休后不再通过安装器引用整个世界，也不取消可能共享的底层 I/O。
- 专用 `RetiredChunkException` 在原版生成状态链中转为 `UNLOADED`，不会制造空区块或触发延迟崩溃。
  普通 I/O 错误继续走现有恢复流程，其他生成异常仍保留原版失败处理。

安装过程不进入 `RegionContext`，因为原版安装仍可能同步请求其他 cell，且同步请求会通过 `managedBlock()` 消费世界事件循环。
当前接入验证的是生命周期和发布顺序；实体、计划刻、POI 内部状态及距离管理仍归串行世界线程。
已经进入的安装回调允许执行完毕，不通过中断或回滚撤销副作用。

同时移除了原读取失败处理中的空条件分支及其无用变量、导入；没有扩大“遇到异常就生成空区块”的适用范围。

### 真实重载发现的生成状态混淆

隔离服首次生成、保存和卸载均成功，但同坐标重载卡在同步区块请求中。针对性回归复现了已有调度器的问题：
较小任务读取一个存档状态为 `FULL` 的区块后，把这个持久化状态当成当前运行时已经完成的加载状态，
因而清除了后续更高目标的请求，留下永远不完成的 Future。

`ChunkGenerationTask` 完成任务时现在报告本次 Future 实际对应的状态。存档中的高级状态只用于选择加载或生成步骤，
不能替代当前运行时仍需执行的光照初始化、光照加载和 FULL 转换。
同时在构建加载图之前捕获目标状态，避免构建期间发生目标升级后，把旧 Future 错标为新目标而跳过步骤。
两个回归分别验证持久化 FULL 区块的后续加载请求，以及加载图构建期间的目标升级；均已先观察到旧代码失败。

## 第五批：NMS 实体与方块实体 tick 成员分 cell

新增 NMS `CellTickList`，实体以 ID 索引、方块实体 wrapper 以对象身份索引；成员主体存放在 cell 容器中。
cell 使用 `ChunkMap` 的同一坐标换算规则，不保存会随拆分/合并失效的 region ID。
串行遍历使用注册顺序的不可变缓存，只有新增、移除或对象替换才使缓存失效；移动同一个对象不会重建整份遍历。
需要重建时按各 cell 的注册序号归并，保持原有跨 cell 顺序，避免直接按地理顺序遍历改变玩法。

- `EntityTickList` 不再维护全世界 active/passive 双 Map。section 回调将仍参与 tick 的实体迁入当前 cell；停止 tick 后的移动不重新注册。
  一次遍历保留开始时的成员快照；回调中的新增和替换在下次遍历生效。旧对象的移除或移动不能误操作复用同一 ID 的新对象。
- `BlockEntityTickList` 统一管理 active、pending 和遍历状态，两个成员容器都按 cell 存放。
  遍历中新注册的 ticker 延至下一轮；移除后当前遍历剩余部分也不会执行它。重复注册同一个 wrapper 不产生重复 tick。
- `LevelChunk` 移除单个方块实体或清理全部方块实体时，重新绑定空 ticker 后立即从 active/pending 注销。
  注销按已注册身份和 cell 定位，不会被空 ticker 的 `BlockPos.ZERO` 误导；卸载不再等待后续世界 tick 清理残留引用。
- 遍历状态通过 `finally` 恢复。异常继续向上传递，不会把后续新增永久送入 pending；重入模拟明确失败，诊断遍历允许读取稳定快照。
- `ServerLevel.fand$simulationCells()` 提供世界控制线程上的只读诊断：每个 cell 的实体、活动/待加入方块实体数量及当前区域 ID。
  区域 ID 只描述布局，不能授予线程执行权限。空容器不保留空 cell。

主体 `fand-api` 接口保持不变；本批改变了 NMS 内部容器类型，直接链接这些内部字段的代码需要重新编译。
实体 AI、载具与乘客递归、冻结判断及执行顺序仍沿原版 tick 路径运行；成员 cell 不等于已经解决乘客跨边界共同执行。
计划刻、邻居更新、随机源、POI 和实体存储发布仍待拆分，因此本批没有开启 NMS 区域并行。

## 第六批：计划刻、block events 与邻居更新链

### 方块与流体计划刻

`LevelTicks` 使用 `CellTickContainerIndex` 管理区块容器，每个 cell 保存自己的区块索引及截止时间集合。
每个容器最多有一个截止时间条目，每个非空调度 cell 最多有一个汇总条目；更新时直接移除旧条目再插入新条目。
移除了全世界多份 next/queued Map、过期条目、堆膨胀阈值及整堆重建路径。
方块计划刻使用截止时间索引；流体的原有配置仍可切换索引和扫描方式，切换不再复制或重建调度状态。

一次 `tick()` 的待执行列表、已执行记录和查询缓存属于 `TickBatch`，结束或失败后释放。
跨 cell 收集继续使用原版 `INTRA_TICK_DRAIN_ORDER`，保留优先级、sub-tick 顺序和全世界的处理上限；
执行过程中新增的计划刻仍等到下一次收集。`copyArea` 保留复制已执行、本批待执行及未来计划刻的原有语义。

同时修复以下生命周期问题：

- 替换区块容器时先解绑旧回调、退休旧批次成员；旧对象后续新增不能污染新容器的截止时间。
  `LevelChunk` 注销时同时传入自身容器身份，旧区块的迟到注销不能移除同坐标的新容器。
- `clearArea` 同时清理容器、本批成员和 `willTickThisTick` 查询缓存，不再查询到已经取消的计划刻。
- 收集时在 `LevelChunkTicks` 中预留计划刻，直到执行开始才释放；`pack()` 包含尚未执行的预留，
  覆盖原版“先保存，再注销区块”的卸载顺序。预留不改变 `hasScheduledTick` 的原有含义。
- 回调失败时，尚未执行的预留归还原容器，当前回调仍按失败传播，不自动重试其可能已经产生的副作用。
  区块退休也归还未执行条目；同位置、同类型已经新注册的计划刻优先，保存与归还都不会制造重复条目。

存档格式和相对延迟编码保持原样。这里没有把世界时间直接替换成不同步的区域时间，也没有允许工作线程直接执行原版方块回调。

### Block events

`BlockEventQueue` 将去重集合放到各 cell，只保留每个 cell 的最早序号用于串行归并。
跨 cell 按原提交顺序执行；回调中新加入的事件仍在本轮处理，不改成下一 tick 的异步消息。
不能 tick 的位置延后到下一轮，区域清除和区块最终卸载同时清理队列与当前延后列表，避免清除后重新加入或永久残留。
异常继续传播，尚未执行和已延后的其他事件保留；`BlockEventData` 固定坐标快照，防止复用 `MutableBlockPos` 改坏位置和去重键。

### 同步邻居更新链

`CollectingNeighborUpdater` 的栈、当前层新增和更新限额归入一次 `UpdateChain`；所有嵌套调用，包括跨 cell 的调用，共享该同步链。
链结束或异常时统一归还并清空缓冲，下一条链复用容量，避免每次方块更新重新分配栈数组。
保留深度优先顺序、多方向更新的暂停/恢复顺序和原来的链长限制；无限模式不再累计无用计数，有限模式到上限后计数不继续增长。
同时修复简单邻居更新在排队时没有固定可变坐标的问题。

当前一个世界仍使用同一个 updater。后续需要把 updater 交给执行所有者，并在同步链修改方块前解决访问范围；
本批没有用逐 cell 排队来改变红石传播顺序。

`fand$simulationCells()` 已增加每个 cell 的方块计划刻、流体计划刻和 block event 数量，主要公开 Fand API 无需迁移。

## 第七批：真实 NMS 方块实体区域并行

### 派发与发布边界

`ServerLevel.tickBlockEntities()` 接入 `RegionBlockEntityTicker`，按真实 holder 拓扑取得本批区域身份。
同一区域保持原注册顺序，不拆开同一更新链；两个以上可执行区域由服务器持有的 `RegionSimulationWorkers` 并行派发。
每个工作线程同时持有 `RegionContext` 和本次原生阶段的 `RegionTickScope`。模型许可本身不能绕过原生访问检查。

尚未接入的 ticker 所在组在控制线程执行。控制回调可能加载区块或改变布局，所以正式派发前重新解析区域，避免使用退休身份。
区块 holder 发布、最终卸载、实体结构索引维护、存档和插件生命周期仍在控制阶段处理。
主线程等待期间不泵入普通服务器/区块事件循环，避免等待时安装、卸载正在模拟的对象。
任一区域失败也先等待本批所有工作结束，再发布已产生的管理通知并传播错误；不重跑已经产生副作用的 tick。

原生子阶段使用 `RegionTickExecutor.forPhases()`，不领取模型 mailbox，也不推进完整区域时钟。
否则每加入一个并行子阶段都会让延迟任务多走一次时间，这与未来整区域 tick 的含义冲突。

### 原生共享状态调整

- 区域线程直接查询归属范围内已安装的完整区块，不使用世界共享的四项区块缓存，不转投主线程并阻塞。
  需要同步加载尚未准备好的区块或访问其他区域时明确失败；后台生成中的 proto chunk 不授予实时模拟访问权。
- `BlockEntityTickList` 只在成员增删、pending 与快照交接时持有短锁，执行原生回调时不持有世界级锁。
  添加仍延至下批，移除立即影响本批后续成员；空 ticker 不会因其占位坐标落在其他区域而误触发访问检查。
- 邻居更新栈与随机源绑定原生区域作用域；区域内保持同步深度优先传播。红石线的临时信号屏蔽改为查询内状态，
  并在嵌套和异常时恢复，消除全局方块单例上一个区域关闭另一区域信号的问题。
- 计划刻容器的截止时间汇总更新与 block event 登记使用短期索引锁，原生方块回调不在锁内执行。
  世界 sub-tick 序号分配保持唯一，同一区域内的顺序不变；不同区域的登记交错取决于实际执行次序。
- 方块更新的导航/网络通知、区块存档索引变脏通知在本批全部完成后发布。
  漏斗拾取物品立即修改其拥有的实体并标记移除，实体结构索引删除在阶段结束后执行，使其他区域的索引遍历保持稳定。
- 漏斗的平面槽位缓存改为类初始化时生成的只读数组；未初始化的箱子/容器战利品路径留在控制阶段，
  避免多个区域推进同一份存档随机序列。已支持的原生类型与回调所属方块均按具体类型检查，不把自定义 ticker 自动视为兼容。
- 方块组件读取使用控制阶段取得的稳定存储视图，区域写入先记录本区域修改，使同次调用能读到新值，再于阶段结束时写回存档数据。
  `FandBlock` 和方块组件容器可在合法区域内同步使用；全局阻塞线程桥会报告需要异步调度。

配置 `regionSimulation.workers` 默认 `0` 自动选择 2–8 个工作线程，`1` 使用原串行路径；修改需要重启。
主要 API 签名保持不变，相关事件的 JavaDoc 已改为模拟所有者线程。插件迁移要求见 [原生区域回调](API_DEVELOPMENT.md#原生区域回调)。
区域随机源独立推进，不承诺与串行模式逐次相同的随机序列。

`ServerLevel.fand$regionExecutionSnapshot()` 提供只读诊断：已完成并行阶段数、ticker 派发数、峰值同时运行区域数，
以及最近一批最多 32 个区域的线程名与开始/结束时间。该快照只能在控制线程读取，不代表当前执行许可。
派发数包含因移除或当前不应模拟而跳过的成员；最近一批记录在卸载后保留，不能据此判断当前区域仍然活动。

## 第一批验证结果

已有调度器与线程桥测试作为改造前基线；新增回归覆盖后台提交、提交顺序、下一 tick、异常传递、
取消、关闭与提交竞争、关闭时已经取出的任务、失效世界、实体移动/移除/替换、玩家句柄重绑定和插件停用。
关联回归同时覆盖 API、插件生命周期与指令系统。

第一批新增 24 个回归用例。验证结果为 API 198 项、相关服务端 176 项全部通过，零失败、零跳过；
独立 `test-plugin` 工程也已重新编译通过。执行命令：

```powershell
./gradlew.bat :fand-api:test :fand-server:test --tests "io.fand.server.scheduler.*" --tests "io.fand.server.plugin.*" --tests "io.fand.server.command.*" --tests "io.fand.server.util.ServerThreadingTest" --console=plain
./gradlew.bat -p "test-plugin" compileJava --console=plain
```

这批是调度器与包装器层面的真实实现测试，NMS 目标使用 mock 隔离服务器启动。
尚未用实际在线玩家验证区域并行，也尚无多核性能结论。正式的服务器集成场景、机器基准、旧插件二进制样本和所有权压力模型仍是 P0/P1 后续工作。

## 第二、三批验证结果

第二、三批统一验证已完成：API 198 项、服务端 717 项、已有集成检查 3 项全部通过，零失败、零跳过。
服务端测试包含 69 项区域模型测试，覆盖随机拓扑变更、并行执行、holder 替换竞争、延迟换算与关闭。
另外将 4 处依赖清理实现中局部变量名的源码断言移除，使用实际资源清理行为测试及已有服务注册表测试覆盖。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest :fand-server:assemble --console=plain
```

完整构建成功，生成 `fand-server/build/libs/fand-server-0.8.4-clip.jar`，以及主 JAR、源码包和 Javadoc 包。
已有集成检查仅验证构建信息与源码挂接，不是在线玩家或真实分区世界的运行测试。

## 第四批验证结果

生命周期接入完成后，全量 API 198 项、服务端 733 项、已有集成检查 3 项通过；完整构建成功。
随后真实重载发现的两处生成调度错误已加入回归，117 项区域模型、区块生成和并行派发相关测试全部通过。
本批新增 18 项用例：串行生命周期与发布 13 项、NMS 状态链及事件循环 3 项、磁盘状态和目标升级 2 项。

修复后的定向验证与启动包构建：

```powershell
./gradlew.bat :fand-server:test --tests "io.fand.server.tick.*" --tests "net.minecraft.server.level.Chunk*Test" --tests "net.minecraft.server.level.FandAsync*Test" --tests "net.minecraft.server.level.FandChunk*Test" --tests "net.minecraft.server.level.ParallelChunkTaskDispatcherTest" :fand-server:fandclipJar --console=plain
```

NMS 变更已先提交内层 Git，再由 paperweight 生成 `0107`、`0108` 两个 feature 补丁。
本批仅新增 feature 提交，内层工作区干净，file/resource 层未改动，因此使用以下专用任务，避免无关的上游基线重建：

```powershell
./gradlew.bat :fand-server:rebuildFeaturePatches -x :fand-server:rebuildFilePatches --console=plain
```

有 file/resource 层改动时应使用完整重建流程，不能照搬上述排除项。

隔离服使用 Java 25、独立测试存档、仅本机监听、2 个区块生成线程和 2 个后台线程。
运行探针通过内部 NMS 快照读取 holder、cell 和区域数量，主世界验证过程如下：

| 操作 | holders | 活动 cells | regions |
| --- | ---: | ---: | ---: |
| 加载两个相隔较远的 4 × 4 区块组，含生成依赖邻域 | 1800 | 128 | 2 |
| 保存并移除全部强制加载，等待最终卸载 | 0 | 0 | 0 |
| 从磁盘重载原坐标 | 1800 | 128 | 2 |
| 只卸载其中一片，另一片继续活动 | 900 | 64 | 1 |

重载后使用 `execute if block` 验证手动写入的钻石块仍存在，输出 `OWNERSHIP_RELOAD_DATA_OK`。
带着剩余 900 个 holder 执行 `stop`，全部维度保存完成并输出 `Fand runtime stopped`，进程退出码为 0。
初次试运行暴露的卡住及修复经过见上文；这些结果不代表已验证在线玩家、实体跨区或多核吞吐。
本机 OSHI 读取 Windows 性能计数器的既有报错仍存在，与本批所有权及加载检查分开记录。

## 第五批验证结果

新增 20 项行为回归：通用 cell 容器 8 项、NMS 实体接入 2 项、方块实体生命周期 9 项、当前区域归属查询 1 项。
覆盖跨 cell 顺序、遍历期间移动/移除/替换、旧对象回调、pending 延迟及卸载、冻结清理、异常恢复和诊断读取。
统一验证结果为 API 198 项、服务端 755 项、已有集成检查 3 项，全部通过，零失败、零跳过；启动包构建成功。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest :fand-server:fandclipJar --console=plain
```

继续使用独立平坦存档、Java 25 和仅本机监听的隔离服，运行真实 NMS tick。探针输出如下：

| 场景 | 观察结果 |
| --- | --- |
| 初始成员 | cell `(156,156)` 有 3 个实体、1 个熔炉；远处 `(187,187)` 有 1 个熔炉，属于另一个区域 |
| 同区域跨 cell 移动 | 盔甲架移至 `(157,156)`，原 cell 实体从 3 减为 2，目标 cell 增为 1 |
| 跨区域移动 | 盔甲架移至 `(187,187)`，中间 cell 索引消失，实体总数仍为 3 |
| 冻结后连续单步 | 盔甲架、矿车、猪乘客的原生 `tickCount` 均依次为 0、1、2、3，没有漏 tick 或重复 tick |
| 两地熔炉 | 放入原铁和煤，单步推进 201 tick；两地输出槽均得到铁锭 |
| 冻结中移除熔炉 | 不再推进模拟，移除左侧熔炉后，其 cell 方块实体数立即变为 0 |
| 保存并最终卸载 | 解除强制加载、恢复正常 tick 以完成票据过期和卸载；三个维度的模拟成员均为空，holder / cell / region 数均归零 |
| 从磁盘重载 | 原来 3 个实体重新注册，矿车与乘客关系保留；仅右侧熔炉恢复，输出槽铁锭仍在，归属查询返回新的区域身份 |

烧炼和重载通过原版 `execute if items block ... container.2 minecraft:iron_ingot` 检查，
输出 `SIMULATION_SMELT_LEFT_OK`、`SIMULATION_SMELT_RIGHT_OK`、`SIMULATION_RELOAD_ITEM_OK`。
重载后执行 `stop`，全部维度保存完成，输出 `Fand runtime stopped`，进程退出码为 0。
冻结期间仍存在尚未最终卸载的 holder 及方块实体属于原有票据生命周期，清零检查在恢复正常 tick、最终卸载后进行。
这些测试验证真实串行玩法和生命周期，尚无在线玩家或 NMS 多核吞吐结论。

内层提交 `65445b8b` 通过 paperweight feature 任务生成 `0109` 补丁，两者的稳定 patch-id 一致；没有手工编辑补丁。

## 第六批验证结果

新增 30 项行为回归：计划刻 15 项、block events 9 项、邻居更新链 6 项。
计划刻覆盖索引与扫描两种模式、跨 cell 优先级及世界处理上限、回调期间新增/清除/替换、异常归还、
预留计划刻保存、复制及 1000 步确定性随机操作；邻居更新覆盖跨 cell 深度优先顺序、方向顺序、限额及 5001 层更新链。

统一验证结果为 API 198 项、服务端 785 项、已有集成检查 3 项，全部通过，零失败、零跳过；启动包构建成功。
随后补齐 `LevelChunk` 按容器身份注销的接入，127 项计划刻、事件、邻居更新、方块实体及区域模型定向回归全部通过，最终启动包重新构建成功。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest :fand-server:fandclipJar --console=plain
./gradlew.bat :fand-server:test --tests "net.minecraft.world.ticks.*" --tests "net.minecraft.world.level.BlockEventQueueTest" --tests "net.minecraft.world.level.redstone.CollectingNeighborUpdaterTest" --tests "net.minecraft.world.level.block.entity.BlockEntityTickListTest" --tests "io.fand.server.tick.*" :fand-server:fandclipJar --console=plain
```

继续使用独立平坦存档、Java 25 和仅本机监听的隔离服，两片相隔较远的区域运行真实 NMS 回调：

| 场景 | 观察结果 |
| --- | --- |
| 冻结与计划刻到期 | 两地 delay=2 的中继器在冻结及推进 3 tick 后仍关闭，第 4 tick 同时点亮并触发活塞；此时水传播仍未到期 |
| 活塞跨 cell | 再推进 4 tick，两地活塞头和推动的石头均出现在相邻 cell；移除电源后，两地活塞正常收回 |
| 流体跨 cell | 有边界的水槽中，水按延迟流入相邻 cell，未因容器分 cell 提前执行 |
| 跨区域复制 | `/clone` 复制仍有待执行计划刻的中继器和活塞；复制后前 3 tick 保持关闭，随后按原延迟点亮并推动活塞 |
| 保存、最终卸载与进程重启 | 探针登记延迟 40000 tick 的原生方块计划刻；保存并最终卸载后停服，使用最终启动包重启并加载原坐标，`SCHEDULED_RELOAD_OK` 确认计划刻恢复，方块及红石状态检查也通过 |
| 再次最终卸载 | 重启后再次移除强制加载，三个维度的模拟队列均为空，holder / cell / region 数均归零 |

两次测试服均通过 `stop` 正常保存退出，进程退出码为 0。
过程记录保存在本地 `build/region-scheduled-smoke.log` 和 `build/region-scheduled-restart.log`。
这些检查覆盖串行玩法、存档与队列生命周期；尚未验证在线玩家负载，也不构成 NMS 区域并行或多核吞吐结论。

内层提交 `246ba88b` 通过 paperweight feature 任务生成 `0110` 补丁，两者的稳定 patch-id 一致；没有手工编辑补丁。

## 第七批验证结果

新增 12 项行为回归：原生派发与访问范围 8 项、红石查询状态隔离 2 项、并发计划刻登记 1 项、子阶段时钟 1 项。
覆盖两个区域同时进入、同区域顺序、控制阶段合并后重新分组、移除与 pending、失败后等待所有回调退出、
阶段结束后发布、单工作线程串行路径、跨区域/世界访问拒绝，以及组件同次读写和延后持久化。
并发计划刻检查两个区域共登记 4000 项，没有丢失；红石检查并发查询、嵌套及异常恢复。

统一验证为 API 198 项、服务端 796 项、已有集成检查 3 项，全部通过，零失败、零跳过；启动包构建成功。
随后补入组件读写回归，原生派发测试的 8 项定向用例全部通过，最终启动包再次构建成功。
前一次全量执行包含本批前 11 项新增用例，最后新增的组件用例由后一次定向执行验证。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest :fand-server:fandclipJar --console=plain
./gradlew.bat :fand-server:test --tests "net.minecraft.server.level.RegionBlockEntityTickerTest" :fand-server:fandclipJar --console=plain
```

真实验证继续使用 Java 25、独立平坦存档及仅本机监听的隔离服，启用默认区域工作线程配置。
两个强制加载区块组分别位于 `(10000,10000)` 与 `(12000,12000)` 附近，属于同一世界的两个独立区域。

| 场景 | 观察结果 |
| --- | --- |
| 真实原生回调重叠 | 两个新熔炉的 `FurnaceBurnEvent` 分别进入 `Fand Region #1`、`Fand Region #2`，在测试插件的双向屏障内互相等待；只有两个回调同时进入才放行，均输出 `NATIVE_PARALLEL_OVERLAP_OK` |
| 区域内同步 API | 屏障回调直接读取所属 `Block.air()` 与 `Block.blockEntity()`，未转投等待中的控制线程；两地熔炉随后点亮 |
| 漏斗拾取 | 两地漏斗分别拾取 8 个金锭、8 个钻石，物品实体均从世界中移除，输出 `PARALLEL_HOPPER_PICKUP_OK` |
| 跨 cell 传输 | 两个漏斗分别向相邻 cell 的箱子传输物品，均保持同区域所有权，输出 `CROSS_CELL_PARALLEL_TRANSFER_OK` |
| 连续推进 | 累计单步推进 200 tick，两个新熔炉都产出铁锭，两地箱子物品正确；记录 200 个并行阶段、998 次 ticker 派发，峰值同时执行区域数为 2 |
| 单侧卸载与重载 | 右侧最终卸载后只剩一个活动区域；重载后熔炉和箱子物品保留，待执行计划刻恢复，右侧区域身份由 135 变为 262；再次单步后两个新旧区域正常并行 |
| 最终启动包重启 | 正常停服后换入最终构建包，重启并加载两地；`PARALLEL_RESTART_INVENTORIES_OK` 和 `SCHEDULED_RELOAD_OK` 确认物品及计划刻恢复，再推进一 tick，两个工作线程再次重叠执行 5 个原生 ticker |
| 最终卸载与停服 | 两次进程均解除全部强制加载，等待最终卸载后，三个维度的模拟成员及 holder / cell / region 数全部归零；`stop` 正常保存退出，退出码均为 0 |

屏障位于真实熔炉原生回调内部；线程名和时间区间是辅助诊断，重叠结论并非来自独立调度器模型。
探针只用于本地隔离测试，不包含在发布插件或服务端运行路径中。
运行记录保存在本地 `build/region-parallel-smoke.log` 与 `build/region-parallel-restart.log`。
本机 OSHI 读取 Windows 性能计数器的既有启动报错仍存在；插件启用后未出现原生区域执行错误。

内层提交 `440710ee` 通过 paperweight feature 任务生成 `0111` 补丁，两者的稳定 patch-id 一致；没有手工编辑补丁。
这一批验证了方块实体阶段的真实区域并行。实体 tick、随机刻、刷怪及计划刻执行仍在控制阶段，
尚无在线玩家压力测试、多核吞吐提升或完整区域独立 TPS 的结论。

## 第八批：扩大原生生产方块并行范围

酿造台与营火（含灵魂营火、点燃烹饪和熄灭冷却）接入原有区域执行链，仍以具体原生方块和方块实体类型核对支持范围。
营火配方检查器由每个 ticker 独立创建，酿造配方表在此阶段只读；不引入全世界互斥锁，也不放开实体 tick 的线程访问。

- `Containers.dropItemStack` 在区域内先验证掉落位置归属，再消费源栈并保存独立物品快照。
  真正的物品实体构造、ID 分配和实体索引注册留到全部区域结束后，避免营火产物、酿造材料容器余物并发修改实体管理器。
  原来在控制线程调用的路径保持不变；区域内刚产生的掉落物从下一次模拟开始可供漏斗拾取。
- `ServerLevel.levelEvent` 捕获不可变坐标，在阶段结束后广播音效；`gameEvent` 在同一发布队列中分发振动事件。
  这保留同一区域的发布顺序，也使监听器回调不会在另一区域仍运行时修改其状态。
- 新增带位置的发布入口 `RegionTickScope.deferAt`，跨区域或跨世界调用在登记前失败。
  掉落位置校验发生在消费物品前，拒绝调用不会吞掉源物品。
- `BrewEvent`、`BrewingStandFuelEvent` 仍同步返回取消及结果修改，但独立区域监听器可能并发，API 文档与配置说明同步更新。

### 第八批验证结果

新增 4 项回归，覆盖并发容器掉落的数量及快照隔离、跨区拒绝时保留源栈、广播捕获可变坐标、游戏事件在控制线程按发布顺序分发。
全量 API 198 项、服务端 801 项、已有集成检查 3 项通过，零失败、零跳过；启动包构建成功。
依赖仓库首次请求返回 HTTP 502，使用本机已经缓存的依赖完成以下验证，没有更换依赖或降低检查范围：

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest :fand-server:fandclipJar --offline --console=plain
```

真实隔离服使用同一世界的两个独立区域，保留上一批的熔炉和漏斗，同时加入酿造台、普通营火与灵魂营火：

| 场景 | 观察结果 |
| --- | --- |
| 酿造回调并行 | 两地 `BrewingStandFuelEvent` 在双向屏障中同时进入不同 `Fand Region` 线程，均输出 `BREWING_PARALLEL_OVERLAP_OK`；营火同时位于这两个并行组中 |
| 营火烹饪 | 普通营火与灵魂营火分别清空原料槽并产生一份正确熟食，输出 `CAMPFIRE_PARALLEL_OUTPUT_OK` |
| 保存及进程重启 | 两地酿造剩余 399 tick 时保存退出；重启后自然续酿完成，均生成粗制药水，原料耗尽、燃料剩 19，输出 `BREWING_RESTART_OUTPUT_OK` |
| 酿造容器余物 | 两地各用一份龙息将喷溅药水变为滞留药水；原先两份龙息剩一份，并各产生一个玻璃瓶实体，输出 `BREWING_REMAINDER_OK` |
| 熄灭冷却 | 两地熄灭营火的烹饪进度单步从 5 降到 3，输出 `CAMPFIRE_COOLDOWN_OK` |
| 游戏事件 | 在两地营火注册真实游戏事件监听器，烹饪完成后均在 `Server thread` 接收通知，输出 `CAMPFIRE_EVENT_ON_CONTROL_OK` |
| 掉落发布边界 | 营火本批完成时下方漏斗仍为空、掉落物已发布；下一 tick 漏斗分别收到熟鸡肉和熟鳕鱼，物品实体移除，输出 `DROP_PUBLICATION_BOUNDARY_OK` 与 `NEXT_TICK_HOPPER_PICKUP_OK` |

重启后诊断累计记录 933 个并行阶段、8407 次 ticker 派发，峰值并发区域数为 2；这不是吞吐性能对比。
解除全部强制加载并等待最终卸载后，三个维度的模拟成员和 holder / cell / region 数均归零。
两次进程均正常保存退出，退出码为 0；本机 OSHI 性能计数器的既有启动报错未在本批处理。
构建记录为本地 `build/region-production-expansion-build-offline.log`，运行记录为 `build/region-production-smoke.log` 与 `build/region-production-restart.log`。
内层提交 `78a3b8c1` 通过 paperweight 生成 `0112` feature 补丁，稳定 patch-id 一致；未手工修改补丁。
实体 tick、随机刻、刷怪及计划刻执行仍未迁入区域工作线程，本批的实体注册延后发布不等于实体模拟已并行。

## 第九批：原生计划刻分区执行

`RegionTickDispatcher<T>` 统一处理本批区域归属、控制组执行后的拓扑重读、区域许可、阶段屏障、失败排空和延后发布。
方块实体与方块计划刻使用各自的适配器和诊断实例，复用相同执行规则；仍不领取完整区域 mailbox、不推进完整区域时钟。

`LevelTicks` 先按原版到期、优先级和世界预算收集任务，然后把不可变任务清单交给派发器。
批次内的任务以对象身份登记，领取只持有短锁以删除预留和记录执行历史，锁外运行原生回调。
同一个任务令牌只能领取一次；被清除、容器替换或卸载撤销的预留即使还在派发快照里，也不会再执行。

`willTickThisTick` 查询本轮尚未领取的任务；`hasScheduledTick` 查询容器中新登记的任务，语义不混用。
回调中新加入的已到期任务不会进入本轮快照。失败时先排空全部区域，再把尚未领取的预留归还原容器，
已经产生副作用的回调不重跑。清除和复制批次状态使用同一短锁，不在锁内执行方块回调。

区域工作线程查询、清除、复制计划刻时核对实际世界队列与所有权。
范围操作在修改前检查覆盖的每个 chunk；复制先检查源范围和位移后的目标范围，避免跨区失败时只复制了一半。
容器注册/注销、世界级计数和诊断只允许控制阶段使用，不能从工作线程遍历其他区域正在修改的队列。

当前原生并行适配只接入具体 `RedstoneLampBlock` 的延迟熄灭回调；同区域出现其他类型到期任务时，整组走控制线程。
中继器、比较器和侦测器可同步驱动邻居，连锁路径可能点燃 TNT 或创建实体，尚未把这些路径视为线程安全。
流体计划刻继续串行。`tickBlock` 传给回调的随机源改为当前区域作用域的随机源，控制线程路径沿用世界随机源。
`fand$scheduledRegionExecutionSnapshot()` 单独提供计划刻阶段的历史诊断，原有方块实体诊断仍保留。

### 第九批验证结果

新增 6 项计划刻区域执行回归：实际红石灯回调重叠、失败后只归还未开始任务、区域内清除与复制及越界拒绝、
世界预算与下一轮登记语义、未审计类型回退、同一预留只能领取一次。
已有计划刻测试继续覆盖索引/扫描模式、顺序、保存、替换、卸载、复制与确定性随机操作；已有方块实体派发测试同时验证提取后的通用派发器。
全量 API 198 项、服务端 807 项、已有集成检查 3 项通过，零失败、零跳过。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest --offline --console=plain
```

本批未生成新的服务端启动包。隔离服直接加载当前编译输出，复用已有 Minecraft 资源及依赖运行，Java 25、仅本机监听、独立测试存档。
两个独立区域的红石灯位于 `(10032,64,10032)`、`(12048,64,12048)`。
断电后单步推进 3 tick，两灯仍亮，输出 `SCHEDULED_DELAY_PRESERVED_OK`；第 4 tick，
原生状态修改事件分别进入 `Fand Region #7`、`Fand Region #8`，双向屏障同时放行并输出 `SCHEDULED_PARALLEL_OVERLAP_OK`。
随后两灯均熄灭，输出 `SCHEDULED_PARALLEL_LAMPS_OFF_OK`，独立计划刻诊断记录 1 个并行阶段、2 次任务派发、峰值并发区域数 2。
再次断电并推进 2 tick 后，两灯仍亮，保存并正常停服；重启后两灯按剩余计划刻熄灭，输出 `SCHEDULED_RESTART_LAMPS_OK`，
诊断再次记录两个区域重叠执行。之前保存的 40000 tick 长延迟任务也通过 `SCHEDULED_RELOAD_OK` 检查。
重启后解除所有强制加载，最终三个维度的模拟成员及 holder / cell / region 数归零；验证进程正常保存退出。
本机 OSHI 性能计数器的既有启动报错仍存在，没有将它算作本批功能验证成功或失败的依据。

首次隔离运行的探针只过滤横坐标，把上方电源块的控制线程修改误认为计划刻回调，主动断言终止了测试服。
补齐高度过滤后重跑；这一探针问题没有通过放宽服务端线程检查来绕过。
相关失败记录保留在本地 `build/region-scheduled-probe-filter-failure.log`。

测试记录保存在本地 `build/region-scheduled-parallel-tests-final.log`、`build/region-scheduled-parallel-smoke.log` 和 `build/region-scheduled-parallel-restart.log`。
内层提交 `f079fcad` 经 paperweight 生成 `0113` feature 补丁，两者稳定 patch-id 一致，未手工编辑补丁。

## 第十批：红石临时状态隔离与串行回退顺序

移除 `RedstoneTorchBlock` 的静态弱引用世界表及全世界切换记录列表。
每个 `ServerLevel` 持有自己的 `RedstoneTorchHistory`，记录以不可变方块坐标为键；区域合并或拆分不更换键，也不丢失烧毁历史。
每个位置只保留最近 8 次切换，查询和登记只更新该位置的窗口，避免每次火把切换都扫描整个世界。
短暂的 map 原子更新不包含任何方块、邻居或插件回调，不对原生模拟加世界锁。
世界推进时清理过期记录，即使再也没有火把回调，也能释放历史；冻结时不人为推进历史。
保持原版 `time - when > 60` 过期边界、8 次切换阈值及 160 tick 重试延迟。
原生入口在区域作用域中先校验世界与位置，越界失败不会修改历史。

同时修复通用派发器的回退顺序：不足两个可并行区域时直接沿用原始任务清单，
不会把交错的 A1、B1、A2 改成 A1、A2、B1。
需要并行时，控制组之间保持输入相对顺序，待派发候选也保留输入顺序；控制回调导致区域合并后，候选回退仍保持此顺序。
真正并行时不同区域间不承诺全局回调先后，同一区域仍按本轮选定顺序执行。

火把历史隔离只是红石邻居链迁移的一部分。本批没有开放红石火把、中继器、比较器或侦测器的并行计划刻，
也没有把实体出生、TNT 点燃和容器战利品生成视为已完成区域化。

### 第十批验证结果

新增 9 项回归：6 项历史窗口测试覆盖边界、超额切换、位置/世界隔离、可变坐标、闲置清理、
20,000 次确定性操作对照原版算法和并发登记；另 3 项覆盖混合计划刻顺序、控制回调合并区域后的顺序、
真实火把 tick 对区域历史的读写及跨区拒绝。后者隔离了邻居和方块写入接口，不能替代完整红石链的实服并行验证。

针对性运行最初发现两项已有掉落测试缺少物品组件初始化，依赖其他测试先运行；
在测试类中显式初始化原版注册表组件后，27 项针对性检查独立通过。
最终全量 API 198 项、服务端 816 项、集成检查 3 项通过，共 1017 项，零失败、零错误、零跳过。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest --offline --console=plain
```

隔离服直接加载当前编译类，没有生成新启动 JAR。冻结世界后，通过反复替换火把下方支撑块的电源状态，
单步触发真实计划刻；前 7 次关断后均恢复，第 8 次关断后保持烧毁状态，另一区域的火把仍亮，
输出 `TORCH_BURNOUT_ISOLATION_OK`。最后一次关断后第 159 tick 仍熄灭，输出 `TORCH_RETRY_159_OFF_OK`；
第 160 tick 恢复，输出 `TORCH_RETRY_160_RECOVERED_OK`。
解除强制加载后，三个维度的模拟成员及 holder / cell / region 归零，进程正常保存退出，退出码 0。

本地记录为 `build/region-torch-history-tests-final.log`、`build/region-torch-history-regression.log`、
`build/region-torch-history-smoke.log`；首次针对性检查失败记录保留在 `build/region-torch-history-tests.log`。
内层提交为 `37639f4c`，补丁通过 paperweight 自动生成，未手工修改。

## 第十一批：方块事件区域访问与 TNT 点燃发布

`BlockEventQueue` 原先只有 `add` 同步，清除、卸载、轮询和诊断却直接访问同一组 `HashMap` / `TreeMap`。
区域回调并发登记和清除时，可能破坏队列与区域头部索引的一致性。
现在集合读写、延后列表建立及回填共用队列短锁，`shouldTick` 与原生事件回调始终在锁外。
仍逐条领取事件，保留同一趟执行中新登记事件的处理方式；不把整趟事件执行放进世界锁。

队列绑定实际 `ServerLevel`，区域登记必须属于同一个世界和当前区域。
范围清除先检查覆盖的所有 chunk，包括两端同属一个区域但中间没有归属的空洞，验证通过后才删除。
任意谓词删除、事件派发、区块移除和全世界诊断拒绝区域作用域；卸载仍在全部区域结束后的控制阶段进行。
清除谓词抛错时，以 `finally` 修复当前 cell 的头部索引，未删除事件继续可达。
这不回滚已完成的删除，也不重放已经进入执行回调的事件。

`TntBlock.prime` 同步确认 `TNT_EXPLODES` 并返回是否接受点燃，捕获不可变位置并验证当前区域归属。
调用方的方块移除仍立即发生；实体构造、注册、声音和 `PRIME_FUSE` 游戏事件作为一个有序发布动作，
在所有区域结束后运行。连同构造一起后移，是因为实体 ID 分配也会查询世界实体索引，不能只把 `addFreshEntity` 后移。
发布动作不再次读取游戏规则，避免已接受点燃且移除方块后，因为更早发布动作改变规则而丢失 TNT。
原版点燃并不使用实体注册返回值，取消实体生成时继续保留该返回值、方块消耗和效果通知语义。

控制线程直接点燃仍同步执行。区域点燃的实体可见性与 `EntitySpawnEvent` 会延后到阶段发布，
这属于已说明的线程迁移语义，并非保证所有插件观察顺序都与单线程相同。
通用实体注册、爆炸中的 `TntBlock.wasExploded`、方块资源掉落和未初始化容器战利品尚未完成迁移；
因此本批仍没有把中继器、比较器、侦测器或红石火把直接加入并行计划刻白名单。

### 第十一批验证结果

新增 13 项检查：4 项队列并发与异常恢复测试、3 项真实区域作用域下的方块事件入口测试、6 项 TNT 点燃发布测试。
覆盖多个生产者登记/清除/去重、两种回调锁外执行、谓词抛错后的可达性、跨世界/跨区/中间空洞拒绝、
区域内生命周期拒绝、实际 TNT 邻居回调、发布顺序、可变坐标、游戏规则关闭与中途修改、生成取消及同步控制路径。
22 项针对性检查通过；最终全量 API 198 项、服务端 829 项、集成检查 3 项通过，
共 1030 项，零失败、零错误、零跳过。

```powershell
./gradlew.bat :fand-api:test :fand-server:test :fand-server:integrationTest --offline --console=plain
```

实服直接运行编译输出。探针在两地真实红石灯计划刻回调中设置红石块，触发邻近 TNT 的原生邻居更新，
并各登记、清除、重复登记一个音符盒事件。两个回调通过双向屏障同时进入 `Fand Region #7/#8`；
TNT 方块在所属区域立即变为空气，阶段期间实体尚未发布，输出 `NATIVE_TNT_ACCEPTED_AND_EVENTS_QUEUED`。
全部区域结束后，两次实体生成事件在 `Server thread` 上发生，观察到去重后的两个待执行方块事件，
输出 `TNT_PUBLISHED_ON_CONTROL_OK`。随后事件队列排空，两只 TNT 可从实体索引查询，
输出 `NATIVE_TNT_AND_BLOCK_EVENTS_OK` 与 `TNT_NATIVE_ENTITIES_REGISTERED_OK`。
探针将测试 TNT 引信改为 10000 tick，避免验证期间发生爆炸；单步结束后两只均为 9999 tick。

正常保存退出并重新启动后，两处 TNT 都恢复，输出 `TNT_RESTART_ENTITIES_OK`。
清理本批测试 TNT 并解除强制加载后，三个维度的模拟成员及 holder / cell / region 数均归零，两个进程都正常保存退出。
验证不代表 TNT 自身 tick 或爆炸已并行；本批只迁移点燃的发布边界。
本地记录为 `build/region-events-tnt-tests.log`、`build/region-events-tnt-regression.log`、
`build/region-events-tnt-smoke.log`、`build/region-events-tnt-restart.log`。
内层提交 `f52eb58f` 通过 paperweight 生成 `0115` 补丁，两者稳定 patch-id 一致，未手工修改补丁，也未生成新服务端启动包。

## 下一批实施顺序

1. 继续拆分随机刻、刷怪和区域模拟临时状态；扩展红石计划刻前先处理同步邻居链中的实体创建、点燃及其他世界管理副作用，不按单个方块随意拆开红石传播。
2. 扩展串行运行中的所有权审计，覆盖 POI 和实体存储的异步发布；不能把独立模型误当成实时世界授权。
3. 将目标调度路由接到已验证的运行时归属，把局部时钟与延迟迁入原版计划刻，再补实体移交和跟随定时任务。
4. 测量大区域拓扑调整和任务队列开销，再实现有限预算的布局维护与负载策略。
5. 使用固定存档、插件和在线玩家负载建立性能与行为基线，逐阶段扩大原生区域并行范围；完整模拟状态归属明确后再取消世界阶段屏障。
